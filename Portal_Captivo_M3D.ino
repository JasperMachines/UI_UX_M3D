      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            sensorsignal = map(analogRead(34), 0, 4096, 0, 180);
            ...
            // client.println("<div class=\"al\"><form><input type=\"number\" class=\"servonumber\" name=\"SIGNAL\" min=\"0\" max=\"180\" value=\"0\" <script="">");
            // client.println("<input type=\"range\" min=\"0\" max=\"180\" class=\"slider\" id=\"servoSlider5\" onchange=\"this.form.SIGNAL.value=this.value\"  value=\""+sensorsignal+"\"/></form></div>");
            //client.println("<h1>Sensor Value:<span id=\"ADCValue\">0</span></h1><br>");
//            client.println("<input type="number" id="ADCValue" name="sensignal" min="0" max="180" value="+sensorsignal+" <script="">");
            //
          //     client.println("<h1>S<span  id=\"ADCValue\" >0</span></h1>");
         client.println("<div class=\"horizontal\"><form>");
          client.println("<div class=\"vertical\"><button onClick=\"history.go(0);\">GET SIGNAL</button>");
           client.println("<input type=\"text\" class=\"servonumber\" id=\"ADCValue\" name=\"SIGNAL\" min=\"0\" max=\"180\" value=\""+sensorsignal+"\" <script=""></form></div>");
